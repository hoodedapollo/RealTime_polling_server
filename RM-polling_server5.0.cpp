/***************************************************************************************************
 *
 *  Four periodc threads scheduled with rate monotonic, one aperiodic thread.
 *  The polling server is the periodic task with the highest priority among all periodic
 *  tasks, it is in charge of the scheduling of the aperiodic task.
 *  The polling server schedules an aperiodic task as long as it has enough capacity left
 *  to run. If the capacity expires whlie the aperiodic task is still running the polling server
 *  signal the aperiodic task to stop. This is done withe the pthread_kill function and an handler 
 *  which stops on a condition variable.
 *  During its next execution the polling server release the mutex signaling the condition variable.
...
#define TASK_4 1
#define TASK_5 2
#define CAPACITY 1000000 // capacity of the polling server, not enough to complete the aperiodic task 
...
// signal handler
void aperiodic_handler(int signo);
...
// should immediatetly release the CPU 
        ...
        else // if (toggle == ARRIVED || toggle == STOPPED) 
        {
                ...
                if (toggle == ARRIVED)
                {
                // release the mutex on which the aperiodic task is waiting since its arrival
                //  by sending the corresponding condition signal                  
                ...
                if (toggle == STOPPED)
                {
                // release the mutex on which the aperiodic task is waiting inside the signal handler
                // since the task was stopped in the middle of its execution. 
                ...
                pthread_cond_timedwait(&cond_stop_task_4,&mutex_stop_task_4,&Capacity);
                ...
                if ( toggle != COMPLETED)
                {
                        ...
                        // send a signal to the aperiodic thread which will be catch and it will cause
                        // the aperiodic task to wait on a condition variable.
                        pthread_kill(thread_id[4], SIGUSR1);
...
// if the aperiodic task recives SIGUSR1 from the polling server must catch it and execute the handler 
// whih makes it wait on a condition variable which will be signaled by the polling server during its 
// next execution
                signal(SIGUSR1, aperiodic_handler);
//                struct sigaction aperiodic_signal;
//                aperiodic_signal.sa_handler = aperiodic_handler;
//                sigaction(SIGUSR1, &aperiodic_signal,NULL);
...
{
        pthread_mutex_lock(&mutex_signal);
        pthread_cond_wait(&cond_signal, &mutex_signal);
        pthread_mutex_unlock(&mutex_signal);
}
